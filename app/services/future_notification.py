...
class NotificationStockPriceService:
    ...
    def __init__(self):
        ...
        self.machine.add_transition(trigger='start', source='new', dest='in_progress', unless=['is_expired', 'is_done'])
    ...
    async def on_enter_in_progress(self):
        ...
        try:
            ...
            self.scheduler.add_job(self.is_done,
                                   trigger='interval',
                                   seconds=self.created_notification.delay,
...
async def main():
    # y = StockService()
    # stock_on_exchanges = await y.find_stocks_by_ticker(stock=FindStockRq(ticker='YNDX'))
    # print(stock_on_exchanges)
    # stock_item = stock_on_exchanges[0]
    # print(stock_item)
    # price = await y.get_stock_price(StockRq(**stock_item.dict()))
    # print(f'Price is: {price}')
    n = NotificationStockPriceService()
    notify_created = await n.create(StockPriceNotificationCreateRq(
        ticker='YNDX',
        exchange=ExchangeRs(code='MCX', name='MCX', yahoo_search_symbol='YNDX.ME'),
        targetPrice=6000,
        action='Buy',
        endNotification=datetime.now() + timedelta(minutes=4),
        delay=20,
        chatId='411442889'
    ))
    n1 = NotificationStockPriceService()
    notify_created = await n1.create(StockPriceNotificationCreateRq(
        ticker='MOEX',
        exchange=ExchangeRs(code='MCX', name='MCX', yahoo_search_symbol='MOEX.ME'),
        targetPrice=200,
        action='Sell',
        endNotification=datetime.now() + timedelta(minutes=4),
        delay=20,
    ...
    all_n = await n1.get_many(TelegramUser(chatId='411442889'))
    # notify_gated = await n.get_one(StockPriceNotificationReadRq(chatId='411442889', id=notify_created.id))
    # print(result)
    # print(notify_created)
    print(all_n)
